...
function FileInput({ open, setOpen }) {
  const [dragging, setDragging] = useState(false);
  const [file, setFile] = useState();
  const opacity = open ? "opacity-100" : "opacity-0 pointer-events-none";
  ...
  return (
    <div
      className={`absolute inset-0 flex flex-col items-center justify-center px-4 bg-black/10 transition-opacity duration-500 ${opacity}`}
    >
      <div className="relative w-full max-w-md bg-white p-16 rounded-lg">
        <div
          onClick={() => setOpen(false)}
          className=" absolute top-1 left-2 text-gray-500 text-2xl font-bold cursor-pointer"
        >
          X
        </div>
        <div
          className={`border-dashed border-2 rounded-md py-16 text-center ${
        ...
        >
          <>
            <p className="text-2xl text-[#ccc]">Drag and drop file here</p>
            <input
              type="file"
              className="hidden"
            ...
            />
            <button
              className="bg-blue-500 hover:bg-blue-700 text-white font-medium mt-8 py-2 px-4 rounded"
        ...
        <div className="w-full max-h-[50vh] overflow-auto mt-4">
          {file && (
